...
@as_method
def augment_uuid(DF, id_name="uuid"):
    ...
    DF[id_name] = [str(uuid.uuid4()) for i in range(0, len(DF))]
...
def augment_uuid_for_unique_values_in_columns(DF, lst_column, id_):
    ...
    DF_unique = DF[lst_column].drop_duplicates().augment_uuid(id_)
...
df_melted = (
    ...
    .augment_uuid("tree_attribute_uuid")

    # #.assign(tree_attribute_value_uuid=lambda df_: df_['uuid'])
    # .augment_uuid('tree_attribute_value_uuid')
...
df_big_table[["uuid","value_", "tree_attribute_uuid", "tree_type_uuid", "tree_attribute_value_uuid"]]
# dct_df["tree_type_attribute_value_references"]

# # %%
# # 3daead39-7e12-4fb5-8985-8d57db909e06
# (
# dct_df["tree_attributes"]
...
# )
# # %%
# [str(uuid.uuid4()) for i in range(1, len(dct_df["tree_attributes"]) + 1)]
